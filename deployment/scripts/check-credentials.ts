...
import { resolveGeminiApiKey } from '../../packages/infrastructure/src/gemini/resolveGeminiApiKey';
...
async function checkGemini(): Promise<boolean> {
  ...
  console.log(`\n[Gemini] key shape: ${keyShape(key)} · model: ${model}`);
  try {
