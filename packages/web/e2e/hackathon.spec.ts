...
test.describe("Catalog Greenlight hackathon UI (1280)", () => {
  ...
  test("catalog: dozens of rows", async ({ page }) => {
    ...
    await expect(page.getByText(/\d+( of \d+)? titles in ClickHouse/)).toBeVisible({
      timeout: 30_000,
    });
  ...
  test("ingest: unique title appears in catalog", async ({ page }) => {
    ...
    await page
      ...
      .fill("Playwright E2E ingest — sci-fi test title for hackathon verification.");
  ...
  test("ask: 6 completed steps, SQL, and numeric answer", async ({ page }) => {
    ...
    await expect(page.locator(".timeline-step.status-completed")).toHaveCount(6, {
      timeout: 260_000,
    });
  ...
  test("legacy paths never show a blank screen", async ({ page }) => {
    ...
    await expect(
      page.getByText("ClickHouse measures. TypeScript scores. Gemini explains.").first(),
    ).toBeVisible();
...
test.describe("Catalog Greenlight hackathon UI (390 mobile)", () => {
  ...
  test("all routes load without connection errors", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "User Guide", exact: false })).toBeVisible({
      timeout: 30_000,
    });
